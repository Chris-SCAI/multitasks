...
export function TaskFilters({
  ...
  return (
    <div className="rounded-2xl border border-[#1E293B] bg-[#151D2E] p-6 md:p-8 space-y-5">
      <div className="relative">
        ...
        <Input
          ...
          className="h-14 border-[#1E293B] bg-[#0B1120] pl-12 pr-24 text-lg text-white placeholder:text-slate-500 focus-visible:ring-2 focus-visible:ring-violet-500/30"
        />
        {taskCount !== undefined && (
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-base text-slate-500">
          ...
          return (
            ...
            >
              <Badge
                variant="outline"
                className={cn(
                  "cursor-pointer rounded-full px-4 py-2 text-base font-medium transition-all inline-flex items-center gap-2",
                  isActive
                    ? activeColors[opt.value]
                    : "border-[#1E293B] text-slate-400 hover:border-slate-500 hover:text-slate-300"
                )}
              >
                <span className={cn("size-2.5 rounded-full", isActive ? dotColors[opt.value] : "bg-slate-600")} />
                {opt.label}
              </Badge>
            </button>
          );
      ...
      {hasFilters && (
        <Button variant="ghost" onClick={onClear} className="text-base text-slate-400 hover:text-white hover:bg-[#1C2640]">
          <X className="size-4" />
          Effacer les filtres
        </Button>
      )}
