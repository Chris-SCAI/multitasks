...
export function TaskCard({ task, onUpdate, onDelete, domain }: TaskCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group flex items-start gap-5 rounded-2xl border border-[#1E293B] bg-[#151D2E] p-6 md:p-7 shadow-sm transition-all hover:bg-[#1C2640] hover:shadow-md hover:border-violet-500/20",
        isDone && "opacity-60"
      )}
    >
      ...
      <div className="min-w-0 flex-1 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Badge
            variant="secondary"
            className={cn("text-base", PRIORITY_COLORS[task.priority])}
          >
          ...
          {task.dueDate && (
            <span className="inline-flex items-center gap-1.5 text-base text-neutral-300">
